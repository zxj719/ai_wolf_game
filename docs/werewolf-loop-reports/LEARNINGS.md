...
1. **猎人 DAY_SPEECH Step 0**（R56 执行）：猎人写指导区末尾有"开枪前回顾上轮 identity_table：若某人 reason 已记'开枪优先级：高'，优先锁定"——这是错误位置的读取指令，应提升为思维链 Step 0，放在 Step1 之前，让猎人每天从历史开枪优先级出发做 Step3 决策。
2. **实局 smoke test**（55 轮未完成，最高持续优先级）：ECS 不在云端 allowlist；建议用户本地验证 D2+ 狼人 thought 是否出现"根据历史推理表"引用，多轮是否对同一高优先目标持续关注。
