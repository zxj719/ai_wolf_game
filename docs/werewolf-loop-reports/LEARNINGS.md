...
2. **预言家 DAY_SPEECH Step 0**（R57 建议）：预言家 DAY_SPEECH 写指导有"排队查验优先级：①②③④⑤"标注，但思维链没有 Step 0 读取——每天发言前无法系统从历史积累的待验队列起步。注意：预言家 `checksInfo` 已注入确定性知识（已验玩家），Step 0 的作用是读取"未验但已标注优先级"的候选队列，两者互补。
3. **实局 smoke test**（56 轮未完成，最高持续优先级）：ECS 不在云端 allowlist；建议用户本地验证 D2+ 猎人 thought 是否出现"历史推理表，X号是开枪优先候选"等引用历史积累的推理，多轮是否对同一候选保持连贯关注。
