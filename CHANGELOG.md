...
- **深色模式流程图「白底白字」**：颜色原本是 `mermaid.render()` 那一刻烤进 SVG 的属性，页面主题之后再变（系统夜间模式、右上角手动切换、站内 iframe 被外层同步）时，CSS 立刻跟着变而旧 SVG 停在旧调色板。改为用 `--mmd-node / --mmd-node-border / --mmd-cluster` 在 CSS 里覆盖节点与标签颜色，图永远跟随当前主题。`themeVariables` 保留，首屏直接就是对的、不会闪一下再纠正。
- **浅色模式边框看不清**：`--border` `#e0e0e0` → `#b8bfc7`（对白底对比度 1.27:1 → 1.86:1），表格、代码块、侧栏分隔线一并变清晰。

### 新功能

- **返回主页按钮**：讲义右上角按钮列新增第三个 `⌂ 主页`（明暗 → zen → 主页）。用 `<a target="_top">`，讲义嵌在站内 iframe 时会把最外层文档带回首页，而不是只在 iframe 内部导航。

### 文件变更

| 文件 | 操作 | 说明 |
|------|------|------|
| `public/robotics/assets/style.css` | 修改 | `--mmd-*` 变量 + mermaid 配色覆盖 + `--border` 加深 + `.home-btn` |
| `public/robotics/assets/site.js` | 修改 | 创建 `.home-btn`（`target="_top"`） |
| `public/robotics/day0.html`、`day1.html` | 修改 | 同步讲义源仓库最新内容 |

### 验证

- 无头 Edge 实测 day0 全部 **91 个流程图节点**：深色最差对比度 12.07:1、浅色 15.19:1，明暗手动切换后仍然成立（此前的失效路径是切换后 SVG 不跟随）。
- 讲义源仓库同步提交 `b8f041e`，两边内容一致。
