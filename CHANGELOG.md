...
- **`GET /api/tennis/feedback/summary`**：新增公开只读聚合端点，输出总评价数、加权平均星级、按星级/模式/角色分布、最新 20 条用户评论，为评估循环提供真实用户情感数据来源

### 文件变更

| 文件 | 操作 | 说明 |
|------|------|------|
| `workers/auth/tennis.js` | 修改 | 新增 `handleTennisFeedbackSummary` |
| `workers/auth/index.js` | 修改 | 导入 + 路由注册 `GET /api/tennis/feedback/summary` |

---

## [2026-06-14] 探险事件池扩充（第 11 轮评估循环）

### 可玩性优化

- **探险事件池 32 → 38**（新增 6 个事件，3 小游戏 + 3 剧情，每章各 2）
  - 第一章新增：🍳 铁锅颠勺大赛（rhythmMash）、🐱 流浪猫的秘密（剧情）
  - 第二章新增：⚖️ 论道擂台双鼓（dualTiming）、🔯 封印中的灵兽（剧情）
  - 第三章新增：🛸 飞船对接精度训练（gaugeDrop）、⏱️ 时区悖论（剧情）
  - 每章事件池约从 10-11 扩充至 12-13，同一事件重复出现延迟至第 7-10 局（原约第 5-6 局）

### 文件变更

| 文件 | 操作 | 说明 |
|------|------|------|
| `src/modules/tennis/modes/adventure/events.js` | 修改 | 新增 6 个事件，总池 32 → 38 |
| `src/modules/tennis/modes/adventure/__tests__/events.test.js` | 修改 | 事件数量断言 16+16 → 19+19 |

### 技术细节

- 新小游戏事件复用已有组件（rhythmMash/dualTiming/gaugeDrop），零新增运行时代码
- TennisRoute bundle: 109.07 → 111.04 kB（+1.97 kB 纯文本）；477/477 测试全绿
...
- **powerFactor 成本买威力**：重炮 1.30×/扣杀 1.22× ↔ 挑高 0.90×/切削 0.85×，修复"满分重炮打不过低耗挑高球"（用户实测+仿真双重确认：普通玩家 vs 广场舞大妈胜率 0%→96%）
- 大妈心态 99→58（免疫语义归还 twist）；对手发挥上限 1.5→1.35（人类操作溢价）；体力经济放宽（局间 20/盘间 50/切削 +10/惩罚 0.85/0.7）
- 修复后：重炮流派 7.5%→67.5%，家族挑战通关率 0%→28.6%（中等玩家）

### 用户反馈修复

- 绝技发动反馈（金光大字+就绪横幅+一场限一次标注）；手机卡牌两步预览（点选看说明再打出）
- 新卡 4 张（战术暂停/肾上腺素/第二口气/全神贯注——抽卡与回点体系）
- **永久卡牌收藏**（商店购卡入收藏、上限 10、自动组进每局牌库，migration 008 owned_cards）+ 模式页常驻商店入口

### 遥测系统

- `POST /api/tennis/telemetry`（公开+白名单校验）+ `GET /api/tennis/telemetry/summary`（聚合监控）+ D1 `tennis_telemetry` 表；三模式赛后自动上报（角色/胜负/球数/操作倍率/招式分布/CLUTCH）

## [2026-06-11] 网球 v2 追加（D 段）：坚持/刷分小游戏 + 盲盒开盒 + 关键分挑战 + 特效

### 新功能

- **4 个新小游戏**：
  - 坚持类（撑过 10 秒过关，**每玩一次难度 +1**，等级存本机）：🐦 飞翔的网球（Flappy：扇翅穿球拍缝隙）、🛸 躲避发球机（弹幕：拖动躲网球雨）
  - 刷分类（15 秒限时，分数兑数值）：⛏️ 黄金球工（钩爪抓金球避炸弹，分数 1:1 变金币）、🦘 跳一跳（蓄力跳台，正中连击翻倍）
- **关键分挑战**（节奏保护：常规球不动）：仅在盘点/金球触发——招式小游戏替换为坚持类挑战，过关顶格 1.5× + CLUTCH 特效，失败 0.6×。
- **商店盲盒区**：木盒 50💰（过关开精良+装备和卡，败保底普通）、金盒 120💰（难度 +2 起步，开史诗+和强化卡）、矿工盒 80💰（分数直接变金币，60 分加送装备——技术好能回本）。
- **特效奖励感**：ACE 金色全屏闪、效果拔群大字爆出、CLUTCH 金光、鹰眼改判、被克提示；`prefers-reduced-motion` 自动隐藏。
- **4 个新成就**：关键先生（关键分挑战过关赢分）/ 开盒手气 / 黄金矿工（单次 ≥60 分）/ 飞行执照（飞翔等级 ≥5），后端白名单同步。
- 闯关事件池 +4（市场寻宝/御剑穿峰/踏云梯/陨石雨），全部用新游戏且奖励圆回网球。

### 验证

- 新增 9 测试（关键分判定/难度曲线/分数兑换），全量 471 绿；E2E 实测关键分挑战单场触发 4 次、开盒结算/成就/金币链路正确。
- 有趣的副产品：乱拍 Flappy 的 E2E 机器人在关键分全失利导致连败——**关键分的高压设计真实有效**。

## [2026-06-11] 网球公开赛 v2 大版本：宝可梦式对战 + 牌库 + 装备养成 + 双新模式

按 A→B→C 三段分别上线（spec：`docs/superpowers/specs/2026-06-11-tennis-playability-v2-design.md`）。

### A 段 · 对战内核重做（单局快打全面升级）

- **真实网球招式体系**：8 招公共池（重炮平击/高压扣杀/上旋抽击/切削放缓/网前截击/放小球/挑高球/穿越球），每人宝可梦式 4 招位 + 1 专属绝技（七张，效果各异）。
- **网球知识克制链**：核心四循环（重炮→切削→上旋→放小球→重炮）+ 网前组 5 对，克中「效果拔群 1.5×」、被克 0.7×，结算带战术解说文案。
- **体力即资源**：三档惩罚（充沛/疲惫 −20%/力竭 −40% 且重招禁用），切削回体、局间/盘间回体——「这一球拼不拼」成为真决策。
- **小游戏定胜负**：9 个操作小游戏（发球时机条/连点/缩圈/节奏/急停/打地鼠/力度条/方向反应/双判），表现映射 0.5–1.5× 直接乘进威力；时间戳判定不吃帧率，reduced-motion 自动补偿。
- **真实记分 lite**：15-30-40 成局（Deuce/占先封顶 2 次后金球制），先胜 3 局成盘，三盘两胜。
- **StS 式战术牌库**：10 卡 ×2 档（基础/+强化），每球抽 1、手牌上限 5、战术点 ramp（1→3）、弃牌堆洗回。
- **性格化读招**：七人各有出招权重与流派，每球出招前 75% 真实率的小动作提示；对手同样受体力规则约束。

### B 段 · 永久养成 + 家族挑战

- **装备系统**：5 槽（球拍/球鞋/手胶/护腕/幸运挂件）× 4 品质，输赢都掉落、同槽自动保高折现；挂件 5 种特效（多抽卡/克制+0.1/判定窗口+15%/局间回体/ACE 增伤）。
- **商店五项服务**：购卡 3 选 1 / 卡牌升级 / 购装 / **装备品质升级**（50/120/300 金币）/ 移除卡牌。
- **家族挑战模式**：6 站梯度连战（40-50 起每场 +8），赛间三选一（特训/按摩/进店），击败谁解锁谁的绝技（备战可换装），6 连胜加冕家族球王👑；sessionStorage 快照可恢复。
- **成就系统**（8 枚）+ 全网榜「荣誉」列（👑球王次数/🗺️闯关通关）。
- **D1 永久层**：`tennis_progress` 表（migration 007）+ `GET/PUT /api/tennis/progress`（白名单校验：装备/成就/绝技枚举、金币增量封顶 500、通关数单调），登录 D1 双写 + 游客 localStorage，合并策略并集/取大。

### C 段 · 奇幻闯关（roguelite）

- **世界观**：家族奖杯被「网球之神」偷走，三章节点地图追讨（菜市场江湖→修仙界→太空站），每步 1-2 节点选路（对战/奇遇/商店/休息）。
- **6 名离谱对手带规则扭曲**：广场舞大妈（心理免疫）、外卖小哥（小游戏限时 −30%）、太极宗师（重炮 30% 被反弹）、修仙童子（每 3 球渡劫比心态）、BOT-3000（出招 100% 规律）、BOSS 网球之神（每盘换配招）。
- **「圆回来」机制**：4 个异想天开事件小游戏（火锅捞丸子/蒲扇风暴/打坐数息/失重颠球）+ 8 条剧情二选一，奖励全部映射网球能力；最终 BOSS 是正经网球对决。
- **roguelite 双层**：加点/牌库/临时体力上限单次闯关有效；装备/金币/图鉴**失败也保留**——手残玩家靠长线养成磨上去。

### 技术与验证

- 三层纯函数状态机（battleReducer 球级 / scoring 局盘 / ladder·adventureReducer 赛程），随机数全部 action 注入；新增 **127 个 vitest 用例**（全量 466 绿）。
- 三段各自 puppeteer-core E2E + 生产 fingerprint 校验；E2E 还顺手验证了设计：无脑刷重炮的机器人连败、节能策略机器人取胜——体力经济有效。
- 新增文件集中在 `src/modules/tennis/{battle,meta,modes}/`；后端仅 +2 端点 +1 表。
