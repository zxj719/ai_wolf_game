...
test('T7: consecutivePeaceHintGuard 三元表达式在 if 块内声明', () => {
    const ifGuardStart = guardVarBlock.indexOf('if (isPeacefulNightGuard)');
    // R104: consecutivePeaceHintGuard shifted to ~1205 from if start due to tripleConsecutivePeaceHintGuard block; window updated 600→1500
    const ifSection = guardVarBlock.slice(ifGuardStart, ifGuardStart + 1500);
...
test('T15: guardPeaceNightStep 赋值使用 ${consecutivePeaceHintGuard} 前置拼接模式', () => {
    const ifGuardStart = guardVarBlock.indexOf('if (isPeacefulNightGuard)');
    // R104: assignment moved to ~2156 from if start; window updated 1600→2400
...
test('T16: 原 守卫平安夜推断内容保留（命中推断 + 未中推断 + confidence 升 15-25）', () => {
    const ifGuardStart = guardVarBlock.indexOf('if (isPeacefulNightGuard)');
    // R104: single-night hint content moved to ~2207+ from if start; window updated 1700→2500
