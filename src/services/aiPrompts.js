...
【思维链】
${seerDayHistoryStep}
${seerPeaceNightStep}Step1: 我有什么查验信息必须报？（全部报出，不要藏）
...
【思维链】
${guardDayHistoryStep}
${guardPeaceNightStep}Step1: 基于我的守护记录，我能推断出什么？（在 thought 中分析，speech 中不提守护）
...
白天角色分化（与白天发言策略一致）：先开口的担任【主动方】（质疑好人/带话题方向），后开口的担任【低调方】（装中立评委，"我也在观察"语气，保留信任度）。`
             ...
             // R102: NIGHT_WOLF 两连/三连平安夜换刀决策框架（零间接推断——identity_table 直接读取历史刀口，无需票压代理）
             ...
             if (isConsecutivePeacefulNightWolf) {
                 tripleConsecutivePeaceNightHintWolf = isTripleConsecutivePeacefulNightWolf
                     ? `⭕【三连平安夜三阶换刀决策（thought 中完成）】
      ① 确认三夜刀口：查 identity_table 含"→已N${wolfNightThreePrevDay}夜行刀"/"→已N${wolfNightPrevPrevDay}夜行刀"/"→已N${wolfNightPrevDay}夜行刀"条目，列出三夜目标ID
      ② 三路径推断（超集激活：三连时三层全显示）：
         路径A：三夜刀同一目标 → 守卫极可能固定轮守该目标，换刀 confidence 升 35-45；今晚必须换次高威胁目标
         路径B：两夜刀同一目标+一夜不同 → 重复目标连守概率高，换刀 confidence 升 25-35；选两夜均未被守护的目标
         路径C：三夜各不同目标 → 守卫随机轮守无规律，按单夜路径A/B独立评估昨夜刀口被守概率
      ③ identity_table 追加：在本轮刀口 reason 末尾追加"N${wolfNightPrevDay}三连平安夜：[路径A/B/C]"`
                     : '';
                 consecutivePeaceNightHintWolf = `${tripleConsecutivePeaceNightHintWolf}⭕【两连平安夜二阶换刀决策（thought 中完成）】
      ① 确认两夜刀口：查 identity_table 含"→已N${wolfNightPrevPrevDay}夜行刀"（两夜前）和"→已N${wolfNightPrevDay}夜行刀"（昨夜）条目
      ② 两路径推断：
         路径A：两夜刀同一目标 → 守卫很可能连守此目标，换刀 confidence 升 25-35；今晚选次高威胁目标
         路径B：两夜刀不同目标 → 守卫轮守不固定，分别评估两目标守护风险，择低守护概率者
      ③ identity_table 追加：在本轮刀口 reason 末尾追加"N${wolfNightPrevDay}两连平安夜：[路径A/B]"
`;
             }
             const wolfNightPeaceStep = isNightPeacefulWolf
                 ? `${consecutivePeaceNightHintWolf}· 若平安夜 → ⭕【换刀决策框架（thought 中完成）】
...
- 若本轮刀了某目标：在TA的 reason 末尾追加"→已${wolfNightLabel}夜行刀；次日叙事：[预计应对策略]"，标记执行状态并记录次日预案
...
输出:{"useSave":true/false,"usePoison":数字或null,"reasoning":"决策理由(必须包含你的推断：当前大概还剩X好人Y狼人)","thought":"用药思考过程","identity_table":{"玩家号":{"suspect":"角色","confidence":0-100,"reason":"依据"}}}`;
  ...
  阈值提醒：身份暴露后高阈值等待代价过高，≥50% 即可果断殉情`;
             ...
             // R107: NIGHT_DREAMWEAVER 平安夜双来源推断（dream immunity vs guard/witch，单夜→两连→三连一轮完成）
             ...
             if (isConsecutivePeacefulNightDW) {
                 tripleConsecutivePeaceNightHintDW = isTripleConsecutivePeacefulNightDW
                     ? `⭕【三连平安夜三阶双来源推断（thought 中完成）】
     ① 确认三夜入梦目标：查 identity_table 含"N${dwNightThreePrevDay}夜入梦"/"N${dwNightPrevPrevDay}夜入梦"/"N${dwNightPrevDay}夜入梦"的玩家ID
     ② 三路径分析（超集激活：三连时三层全显）：
        路径A：三夜入梦同一目标 → 来源A概率大幅升高（狼持续瞄准），confidence 升 35-45；下轮若再梦须评估是否主动连梦出局该目标
        路径B：两夜入梦同目标+一夜不同 → 两夜来源A概率升，confidence 升 25-35；分别评估两夜守护交集
        路径C：三夜各不同目标 → 无规律，按单夜双来源推断独立评估N${dwNightPrevDay}夜
     ③ identity_table 追加：在本轮入梦目标 reason 末尾追加"N${dwNightPrevDay}三连平安夜：[路径A/B/C]"
`
                     : '';
                 consecutivePeaceNightHintDW = `${tripleConsecutivePeaceNightHintDW}⭕【两连平安夜二阶双来源推断（thought 中完成）】
     ① 确认两夜入梦目标：查 identity_table 含"N${dwNightPrevPrevDay}夜入梦"（两夜前）和"N${dwNightPrevDay}夜入梦"（昨夜）的玩家ID
     ② 两路径分析：
        路径A：两夜入梦同一目标 → 来源A升高（狼两次瞄准被免疫），confidence 升 25-35；评估是否已满足连梦击杀条件
        路径B：两夜入梦不同目标 → 分别评估各夜来源A/B可能性，综合判断守卫保护活跃度
     ③ identity_table 追加：在本轮入梦目标 reason 末尾追加"N${dwNightPrevDay}两连平安夜：[路径A/B]"
`;
             }
             const dwNightPeaceStep = isNightPeacefulDW
                 ? `${consecutivePeaceNightHintDW}⭕【摄梦人平安夜双来源推断（N${dwNightPrevDay}无人死亡，thought 中完成）】
     ① 确认昨夜入梦目标：查 identity_table 含"N${dwNightPrevDay}夜入梦"的玩家ID
     ② 双来源推断（摄梦人专属视角）：
        来源A：狼人瞄准昨夜入梦目标 → 入梦免疫拦截平安夜
           信号：该目标昨日票压较高 OR 历史连续被怀疑 → 来源A可能性升，confidence 升 20-30
           决策：该目标是狼人高优先目标，今晚继续入梦保护价值高
        来源B：狼人瞄准其他目标，守卫/女巫保护生效
           信号：该目标昨日票压低 OR 场上有明显守卫/女巫保护信号 → 来源B可能性升
           决策：今晚可考虑换梦目标，你的入梦保护并非平安夜直接原因
     ③ identity_table 追加：在昨夜入梦目标 reason 末尾追加"N${dwNightPrevDay}平安夜入梦：[来源A-免疫拦截/来源B-守护生效]"
`
                 : '';
...
${dreamweaverNightStyle}
${dwNightPeaceStep}Step1: 场上谁是核心好人？谁是狼头？
...
热力不能直接决定投票——必须先独立评估，再用热力校正：
  ...
  ④ 热力高的人本轮踩明显狼嫌疑 → 好人正确分析特征，热力有效可跟投`;
   ...
   d) 毒亡共谋推断：若毒亡玩家曾在发言中为某存活玩家"金水/力挺"，该存活玩家共谋嫌疑上升——可在 identity_table 中提升其嫌疑并作为投票优先排查对象`
            ...
            // R124: 村民 DAY_VOTE 三维信息聚合框架 — 补齐最后一个无专属策略的好人角色
            // 设计依据（Wang 2025 arxiv:2408.17177）：村民无私有信息，须从发言行为分析合成最优判断；
            // 白熊效应合规：全正向描述（"率先带票/优先锚点/三维打分"），无负向禁令词汇
            const villagerVoteStrategy = `2. 【村民投票—信息聚合三维框架（R124）】村民无私有信息，从综合行为分析中找最优投票目标：
   a) 🔮最高优先锚点：场上有预言家查杀 → 率先跟投（外部验证，信息层级最高）；对跳局面 → 选论据更完整/更早公开/经历史行为验证的那方
   b) 无查杀时：对每个候选在 thought 中完成三维打分（综合得出投票方向）：
      ① 信息价值：提出新推断/有效质疑/清晰逻辑分析 → 高；划水/重复旧话/无实质内容 → 低
      ② 行为一致：当前发言与历史发言/投票方向一致 → 可信；突然改口/回避关键问题/倒钩 → 嫌疑上升
      ③ 逻辑链完整：结论有清晰因果证据链 → 高；结论跳跃/纯猜测堆砌/自相矛盾 → 低
   c) 投票排序：① 预言家查杀（最高信任）→ ② 三维综合最低分存活玩家 → ③ 参考【跨轮投票热力】作为校正信号
...
结尾可带一句底气话——自信本身能说服别人，不要等人问你底牌。`;
             ...
             if (witchSsSaveHave && witchSsPoisonHave) {
                 witchSsMedStatus = '双药均在：竞选核心论点是"双药的主人才是警徽的最佳归宿"——解药+毒药是场上最高价值的未打出资产，1.5票是这两张牌的杠杆放大器。';
             } else if (witchSsSaveHave) {
                 witchSsMedStatus = '解药在手/毒药已用：毒药使用目标是可核实的判断力证明，解药仍在手 = 好人阵营还有一次救援机会，你存活的期望价值远高于一张普通投票。';
             } else if (witchSsPoisonHave) {
                 witchSsMedStatus = '毒药在手/解药已用：若解药曾救人，可报救援记录作信任锚点；毒药尚存 = 定向消灭狼方一次，你出局等于好人失去精准打击机会。';
             } else {
                 witchSsMedStatus = '双药已用：竞选重心是完整药效决策记录——双药使用时机和目标均可核实，是比任何口号更有说服力的判断力证明。';
             }
             const witchSsHint = hasRevealedIdentity
                 ? `你是女巫（已跳身份）：竞选核心 = "信息权威 × 药效执行力"。${witchSsMedStatus}
【竞选3要点（必须包含实质分析）】：
1. 药效决策回顾：简述解药或毒药的使用/保留判断（这是你独有的、可核实的分析记录，不可伪造）
2. 当前局势判断：基于药效信息，你认为最可疑的是谁
${badgeFlowLine}`
                 : `你是女巫（身份未公开）：以"有底气的好人"身份竞选，绝不透露女巫身份或任何药水词汇。
【改编方向（选一个，改成符合当前局势的表达）】：
① "我是那种即便最坏情况发生，好人阵营也不会白亏的人——就凭这个，我应该拿警徽"
② "好的警长应该是场上做了判断就有实质执行力的人——我已经想清楚了，当选之后你们就知道了"
措辞越模糊威慑越强——场上无法判断你的底牌，这个不确定性本身是策略价值。绝不涉及任何身份词汇。
【竞选3要点（必须包含实质分析，不要全靠暗示）】：
1. 昨夜局势分析：你认为最可疑的是谁、最信任谁
2. 隐性底气（从①②改编，1句话）
${badgeFlowLine}`;
             // R112: 守卫 SHERIFF_SPEECH 专属提示词——守护记录可信度 × 持续保护价值
             const guardSsHint = hasRevealedIdentity
                 ? `你是守卫（已公开身份）：竞选核心 = "守护记录可信度 × 持续保护价值"。守护记录是不可伪造的信任锚点——展示核心夜次的守护目标，证明你在读狼人刀法。
【竞选3要点（必须包含实质内容）】：
1. 守护决策回顾：简述你关键夜次的守护目标与判断逻辑（成功拦截=你读懂了狼队，可核实）
2. 当前局势判断：基于守护信息，你认为最可疑的是谁
${badgeFlowLine}`
                 : `你是守卫（身份未公开）：以"稳健理性的好人"身份竞选，绝不透露守卫身份（避免被针对）。
警长核心价值是引导精准投票——展示冷静局势分析，证明你不会被带偏节奏。
【竞选3要点（不含任何守卫信号）】：
1. 昨夜局势分析（最可疑玩家 + 推理依据，展示理性判断而非情绪反应）
2. 你当选后的票型方向（具体到玩家或方向，不要只说"找狼"）
${badgeFlowLine}`;
             // R134: 竞选发言对跳检测——先发言的悍跳者写入 claimHistory 后可被后续候选人检测到
...
语气要自信坚定——你是好人最重要的信息源，理所当然该拿警徽。${seerSsCounterHint}`
                 : playerRole === '狼人'
                 ? `你是狼人：根据你的竞选策略选择发言角度——
...
输出JSON:{"speech":"竞选发言（必须有具体内容）","thought":"真实竞选考量（含博弈思路）","claims":[...身份声明可选；悍跳预言家时填写{"type":"jump_seer","checks":[{"night":1,"targetId":X,"isWolf":true或false}]}；悍跳骑士时填写{"type":"jump_knight","duel":{"targetId":X}}；其他情况省略或留[]]}`;
        }
