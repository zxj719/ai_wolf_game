...
const ENERGY_BG = { low: 'bg-sky-100/70', medium: 'bg-amber-100/70', high: 'bg-rose-100/70' };
    ...
    <div className="mac-panel overflow-hidden p-0">
      ...
      {!ready && total > 0 ? (
        <div className="flex flex-col items-center gap-3 px-6 py-8">
          ...
          <div className="text-sm font-medium text-slate-600">加载音轨中…</div>
          <div className="w-full max-w-xs">
            ...
            <div className="mt-1.5 text-center text-xs tabular-nums text-slate-400">{loaded} / {total} 轨</div>
          </div>
        </div>
      ) : null}
            ...
            {sections.map((sec, i) => {
              ...
              return (
                <button
                  key={i}
                  type="button"
                  className={`relative flex items-center justify-center truncate font-medium ${ENERGY_BG[sec.energy] || 'bg-slate-50'} ${on ? 'ring-1 ring-inset ring-slate-500' : 'opacity-60 hover:opacity-100'} transition-opacity`}
                  style={{ width: `${w}%`, minWidth: '42px' }}
                  onClick={() => seekAll(sec.time_start)}
                  title={`${sec.name} · ${fmt(sec.time_start)}`}
                >
                  <span className="truncate px-0.5 text-slate-600">{sec.name}</span>
                </button>
              );
            })}
          </div>
      ...
      {ready ? (
        <div className="flex items-center gap-3 px-4 py-3">
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-900 text-white shadow-sm hover:bg-slate-700 active:scale-95"
          >
            {playing ? <Pause size={15} /> : <Play size={15} className="ml-0.5" />}
          </button>
          <span className="w-10 shrink-0 text-right text-xs tabular-nums text-slate-500">{fmt(time)}</span>
          <div
            ref={seekBarRef}
          ...
          >
            ...
            {/* Buffer bar */}
            <div
              className="absolute left-0 top-1/2 h-2 -translate-y-1/2 rounded-full bg-slate-200 transition-[width] duration-500"
              style={{ width: `${bufferPct}%` }}
            />
            {/* Progress bar */}
            <div
              className="absolute left-0 top-1/2 h-2 -translate-y-1/2 rounded-full bg-slate-800 transition-[width] duration-75"
              style={{ width: `${pct}%` }}
            />
            {/* Thumb */}
            <div
          ...
          <span className="w-10 shrink-0 text-xs tabular-nums text-slate-500">{fmt(duration)}</span>
        </div>
      ) : null}
          ...
          {stemUrls.map(s => {
            ...
            return (
              <div key={s.name} className="flex items-center gap-2.5 py-1">
                ...
                <span className="w-16 text-xs font-medium text-slate-600">{STEM_LABELS[s.name] || s.name}</span>
                <input
                  type="range"
                  min={0} max={1} step={0.02}
                  value={st.muted ? 0 : st.vol}
                  onChange={e => setMix(p => ({ ...p, [s.name]: { vol: parseFloat(e.target.value), muted: false } }))}
                  className="h-1.5 w-28 accent-slate-700"
                />
              </div>
            );
          })}
