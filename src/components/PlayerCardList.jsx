...
export function PlayerCardList({
  ...
  const getRoleIcon = (role) => {
    switch(role) {
      ...
      default: return <User size={12} className="text-role-villager"/>;
    }
  };
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <div className="w-14 h-14 rounded-full mb-2 border-2 border-white/5 overflow-hidden relative shadow-2xl" style={{backgroundColor: p.avatarColor}}>
                  ...
                  {!p.isAlive && <div className="absolute inset-0 bg-black/60 flex items-center justify-center"><Skull size={24} className="text-role-wolf" /></div>}
              </div>
              <span className="text-[11px] font-black">{p.name}</span>
              
              {/* 显示 AI 模型名称 (精简显示) */}
              {!p.isUser && AI_MODELS.length > 0 && (
                <div className="text-[6px] text-ink-faint mt-0.5 truncate max-w-[80px]">
              ...
              <div className="mt-1 flex flex-wrap gap-1 justify-center">
                {/* 1. 显示用户自己的身份 */}
                {p.isUser && (
                  <span className="text-[7px] bg-success text-ink px-1.5 py-0.5 rounded font-black uppercase flex items-center gap-0.5">
                ...
                {/* 2. 如果用户是狼人，显示其他狼人队友 */}
                {isTeammate && (
                  <span className="text-[7px] bg-role-wolf-soft text-role-wolf px-1.5 py-0.5 rounded font-black uppercase flex items-center gap-0.5">
                    {getRoleIcon('狼人')} 狼同伴
                  </span>
                )}
                ...
                {((gameMode === 'ai-only') || (phase === 'game_over')) && !p.isUser && !isTeammate && p.role && (
                  <span className="text-[7px] bg-accent text-ink px-1.5 py-0.5 rounded font-bold flex items-center gap-0.5">
                ...
                {!p.isAlive && !((gameMode === 'ai-only') || (phase === 'game_over') || p.isUser || isTeammate) && (
                   <span className="text-[7px] bg-bg-raised text-ink-faint px-1.5 py-0.5 rounded font-bold">已死亡</span>
                )}

                {/* 5. 如果用户是预言家，显示查验结果 */}
                {userPlayer?.role === '预言家' && seerChecks.some(c => c.targetId === p.id) && (
                  <span className={`text-[7px] text-ink px-1.5 py-0.5 rounded font-black uppercase flex items-center gap-0.5 ${seerChecks.find(c => c.targetId === p.id).isWolf ? 'bg-danger' : 'bg-role-guard'}`}>
