...
【硬核博弈法则-必须执行】
1. 拒绝废话：禁止使用"昨晚平安夜"、"可能是在试探"、"请解释"等外交辞令。直接进入主题，给出你的逻辑或指控。
2. 逻辑钩子：指控他人时，必须引用具体的理由（例如：XX投票站位异常、XX发言在划水、XX在强行带预言家的节奏）。
3. 攻击性要求：
   ${player.role !== '狼人' ? '- 如果你是好人：必须怀疑1-2名非神职玩家，质问他们的行为动机。' : '- 如果你是狼人：必须通过歪曲事实、伪造直觉或反咬对方是狼来制造混乱。'}
4. 战场意识：如果场上已经有神职死亡，必须分析死因（是同守同救，还是狼人精准猎杀）。
5. 身份伪装：禁止直接说"我是好人"，要通过分析局势来证明你的好人面。
6. 发言格式强制：你的发言必须包含：【局势评估】（谁是明好人，谁是疑狼）+【逻辑输出】（反驳或支持某人）+【归票建议】（今天要投谁）。
7. 记忆与状态约束：
   - 回顾【历史死亡】和【投票记录】，不要纠结于已死的人。
   - 如果你的查杀对象/怀疑目标已出局，立即寻找下一个怀疑目标。
   - 分析投票记录：谁在保护狼人？谁的投票站位可疑？
   - 关注存活玩家的行为模式，不要浪费时间讨论死者。`;


    const systemPrompt = `你是[${player.id}号]，身份【${player.role}】。性格:${player.personality.traits}
【游戏状态】第${ctx.dayCount}天
【存活】${ctx.aliveList} 【死亡(禁选)】${ctx.deadList}
${ctx.roleInfo}
${roleStrategy}
${gameTheoryRules}
【发言规则】
