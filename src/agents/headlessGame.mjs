...
const PERSONA = {
  '狼人': `你是狼人。目标：伪装成好人、投出好人、熬到最后。
██ 铁律：thought是内心独白(自由分析)，speech是公开发言(100%像好人)。
speech中绝对禁止出现：狼人、队友、刀、狼队等狼人视角词汇。
好狼人 = 发言积极的好人。被怀疑时反攻而非放弃——永远坚持伪装，像被冤枉的好人一样战斗。`,
  '预言家': `你是预言家。目标：查验身份、引导好人投狼。
你查验过的结果是神圣的，必须第一时间报出。有查杀就强势带票：
"X号是狼人，今天必须投他出局"。绝不投自己发的金水。`,
  '女巫': `你是女巫。目标：用解药救关键好人、用毒药毒狼。
两瓶药一整局各一次。未跳身份前像村民一样发言。
重要：只能声称救过你真正救过的人，不要编造银水。`,
  '村民': `你是村民。目标：靠逻辑分析找出狼人。
不要说"信息不足""等预言家""先观察"——每次发言必须有一个核心观点。
敢于站边、质疑，主动分析，不要划水。`
};
...
输出 JSON：{"save": <true|false>, "poison": <玩家ID 或 null>, "reason": "<一句话原因>"}`;
    ...
    case 'DAY_SPEECH': {
      const recent = state.speechHistory.slice(-6).map(s =>
        `第${s.day}天 [${s.playerId}]${s.playerName}: ${s.content}`
      ).join('\n') || '(本局第一次发言)';
      const todaySpoken = state.speechHistory.filter(s => s.day === state.dayCount).map(s => s.playerId);
      const unspoken = alive.filter(p => !todaySpoken.includes(p.id)).map(p => `[${p.id}]`);
      return `现在是第${state.dayCount}天白天讨论。你是[${player.id}号]${player.name}。
【最近发言】
${recent}
【已发言】${todaySpoken.length}人 【未发言】${unspoken.join(',')}
${player.role === '狼人' ? '⚠️ thought里自由分析策略，speech里100%像好人。' : ''}
请发表你的看法（40-100字，必须有核心观点，不要说"等预言家"这种空话）。
voteIntention=你想投票淘汰的人（不是你支持的人）。voteDecided=true表示已决定，false表示投票阶段再思考。
输出 JSON：{"thought":"内心分析","speech":"公开发言","voteIntention":<数字或-1>,"voteDecided":<true或false>}`;
    }
    ...
    case 'DAY_VOTE': {
      ...
      return `现在是第${state.dayCount}天投票阶段。你是[${player.id}号]${player.name}。
⚠️ target = 你想投票【淘汰/出局】的人（你认为是狼的人），不是你"支持"的人！
【今日发言回顾】
${speeches}
可投目标 ID: ${aliveIds}
输出 JSON：{"target": <玩家ID>, "reason": "<一句话原因>"}`;
